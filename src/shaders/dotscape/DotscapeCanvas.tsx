...
export interface DotscapeParams {
  speed: number;
  gridDensity: number;
  warpStrength: number;
  dotSize: number;
...
export const DotscapeCanvas: React.FC<{ params: DotscapeParams }> = ({
  params,
}) => {
  ...
  useEffect(() => {
    ...
    const uGridDensity = gl.getUniformLocation(program, "uGridDensity");
    ...
    const loop = () => {
      ...
      gl.uniform1f(uGridDensity, p.gridDensity);
      gl.uniform1f(uWarpStrength, p.warpStrength);
      gl.uniform1f(uDotSize, p.dotSize);
      gl.uniform1f(uBrightness, p.brightness);
      gl.uniform3f(uColorLow, c1[0], c1[1], c1[2]);
      gl.uniform3f(uColorHigh, c2[0], c2[1], c2[2]);
